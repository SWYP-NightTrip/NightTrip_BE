...
public interface ImageRepository extends JpaRepository<ImageUrl, Long> {
    ...
    @Query("SELECT i FROM ImageUrl i WHERE i.imageType = :imageType AND i.relatedId = :relatedId AND i.isMain = true")
    Optional<ImageUrl> findMainImageByTypeAndRelatedId(@Param("imageType") ImageType imageType, @Param("relatedId") Long relatedId);
